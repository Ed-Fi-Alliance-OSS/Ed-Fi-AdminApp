...
- **Ground truth is always live.** Never trust a hardcoded claim about container counts, ports,
  ...
  `docs/adminapp-env/environment-reference.md` is a cache of that, not a second source of truth:
  if validation finds it's drifted from the live files, correct it directly (no confirmation
  needed).
- **Documented-only tags upgrade on exercise.** Every entry in
  `docs/adminapp-env/environment-reference.md` and `docs/adminapp-env/known-issues.md` carries a
  Verified-live vs. Documented-only tag. When you
...
- **Never use unscoped Docker cleanup.** See `docs/adminapp-env/known-issues.md`'s scope-safety entry
...
- **Always scope log-based error checks to a recent window or the latest occurrence.** When
  ...
  `docs/adminapp-env/environment-reference.md` and `docs/adminapp-env/known-issues.md`).
- **Infer expertise/tone from how the user talks; never ask for it directly.** If they ask
  ...
  explanations from `docs/adminapp-env/glossary.md` rather than inventing them inline). If they
  use precise technical language and move fast, skip explanations and act. Check memory for an
  existing `user`-type note about this before the session starts guessing; if the user's current
  behavior clearly contradicts what's stored, update that memory rather than trusting the stale label
...
1. **Read `compose/.env`.**
   - Missing or missing required values (e.g. `SQL_BACKUPS_FOLDER` unset) → walk through
     `docs/adminapp-env/environment-reference.md`'s "One-time setup" section, asking the user only
     for whichever specific values are actually missing. Never re-ask for values already present.
     Once collected, write the missing values into `compose/.env` before proceeding — don't just
     hold them in conversation.
...
2. **Ask, unless the user's phrasing already answered these:**
   ...
   - Separately, if the user wants the Admin App's **own** database on SQL Server instead of
     ...
     "Starting modes" section of `docs/adminapp-env/environment-reference.md` for the mechanics
     (it does not affect the ODS/Admin databases for any topology, which stay Postgres regardless).

3. **Act on the combination:**
   - **Fresh + Container** → run `docs/adminapp-env/environment-reference.md`'s full reset
     recipe, then `start-services.ps1 -Rebuild`, then the full validation checklist (step 5 below).
   ...
   - **Registering an environment in the Admin App UI** (whenever the user's request is this,
     ...
     against the Admin API first. See `docs/adminapp-env/environment-reference.md`'s "OAuth
     client registration" recipe before walking the user through the UI step.

4. **If validation finds something broken:**
   - Check `docs/adminapp-env/known-issues.md` for a matching symptom first — apply its
     documented fix if one matches.
   ...
   - Once a root cause and fix are confirmed → apply the fix, then **ask the user before adding a
     new entry to `docs/adminapp-env/known-issues.md`** describing it (this is different from
     correcting an existing entry that's gone stale, which needs no confirmation).

5. **Validation checklist** (from `docs/adminapp-env/environment-reference.md`, run in this order):
...
`docs/adminapp-env/environment-reference.md`, reflecting what's actually configured (read
`.env`'s DB engine/profile live — don't assume Postgres):
...
from `docs/adminapp-env/glossary.md` rather than writing a new one inline. If an explanation
there is wrong or unclear, fix it directly — that's corrective-maintenance tier.
...
2. If yes, add a new section to `docs/adminapp-env/known-issues.md` following the existing format
