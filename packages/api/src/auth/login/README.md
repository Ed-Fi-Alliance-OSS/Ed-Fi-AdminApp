...
- **Logout CSRF.** Logout is handled by `POST /auth/logout`, so it is covered by the app's CSRF protections and requires the expected same-origin request metadata before it can destroy the local session or initiate IdP logout.
