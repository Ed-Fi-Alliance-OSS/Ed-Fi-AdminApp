...
<#
.SYNOPSIS
Guided Node version remediation. When the active `node` is too old for the
AdminApp, installs nvm-windows (if absent) and switches to a current LTS so
the user can keep their previous Node installable side-by-side.

.DESCRIPTION
Safe to call unconditionally:
  - Node missing from PATH    -> no-op (03a-prereqs-runtime.ps1 will install)
  - Node already >= MinMajor  -> no-op
  - Node too old              -> prompt (or AssumeYes), then run:
        winget install CoreyButler.NVMforWindows  (if nvm missing)
        nvm install $NodeLtsVersion
        nvm use     $NodeLtsVersion

After the upgrade the previous Node version is recoverable via `nvm install
<that-version>` -- nvm-windows manages multiple installs under
%APPDATA%\nvm and exposes the active one via C:\Program Files\nodejs.

.PARAMETER SourcePath
The cloned AdminApp repo. When package.json exists there, engines.node is
parsed and used as the floor + nvm install target. Defaults to the parent
of the script directory (this script lives in <repo>\windows-install\).

.PARAMETER MinNodeMajor
Floor enforced when package.json detection fails. Default: 22.

.PARAMETER NodeLtsVersion
Node version spec to install via nvm. Default: "22" (bare major -- nvm
resolves to the latest patch on that line, which avoids transitive-dep engine
mismatches against an outdated specific patch). When package.json detection
succeeds, this is overridden with the detected major.

.PARAMETER AssumeYes
Switch -- bypass the y/N prompt and proceed with the upgrade. For
non-interactive runs (CI, install-all -AutoUpgradeNode).

.EXAMPLE
.\00a-fix-node.ps1
.\00a-fix-node.ps1 -AssumeYes
.\00a-fix-node.ps1 -NodeLtsVersion 22.11.0
#>
...
# 1. Decide whether action is needed
$node = Get-Command node -ErrorAction SilentlyContinue
if (-not $node) {
    Write-Host "Node not on PATH -- 03a-prereqs-runtime.ps1 will install LTS. Nothing to fix here."
    return
}
...
if ($currentMajor -ge $MinNodeMajor) {
    Write-Host "Node $verRaw is already at or above the required floor ($MinNodeMajor). Nothing to fix." -ForegroundColor Green
    return
}

# 2. Prompt unless -AssumeYes
Write-Host ""
...
# 3. Install nvm-windows if missing
if (Get-Command nvm -ErrorAction SilentlyContinue) {
    ...
    if (-not (Get-Command nvm -ErrorAction SilentlyContinue)) {
        throw "winget reported success but 'nvm' is still not on PATH. Open a fresh PowerShell window (as administrator) and re-run 00a-fix-node.ps1."
    }
}

# 4. nvm install + use the target version
...
# 5. Refresh PATH so child processes inherit the new node symlink target
Refresh-Path
